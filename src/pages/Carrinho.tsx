...
import { MarmitaOrder, PratoOrder, SobremesaOrder, CartItem } from '@/types/order';

export default function Carrinho() {
  const navigate = useNavigate();
  const { state, removeFromCart, updateQuantity, getSubtotal, getCartCount } = useOrder();
  const { cart } = state;
        ...
        <div key={marmita.id} className="card-item">
          <div className="flex justify-between items-start mb-3">
            <div>
              ...
              {marmita.carnesExtras > 0 && (
                <p className="text-xs text-accent">+R${marmita.carnesExtras} extras</p>
              )}
            </div>
            <button 
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <button 
                onClick={() => updateQuantity(marmita.id, Math.max(1, marmita.quantity - 1))}
                className="btn-secondary p-2"
              >
                <Minus size={16} />
              </button>
              <span className="font-bold w-8 text-center">{marmita.quantity}</span>
              <button 
            ...
            <span className="text-accent font-bold text-lg">
              R$ {(marmita.totalPrice * marmita.quantity).toFixed(2).replace('.', ',')}
            </span>
          </div>
        </div>
      );
      ...
      return (
        <div key={prato.id} className="card-item">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <button 
                onClick={() => updateQuantity(prato.id, Math.max(1, prato.quantity - 1))}
                className="btn-secondary p-2"
              >
                <Minus size={16} />
              </button>
              <span className="font-bold w-8 text-center">{prato.quantity}</span>
              <button 
      ...
      return (
        <div key={sobremesa.id} className="card-item">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <button 
                onClick={() => updateQuantity(sobremesa.id, Math.max(1, sobremesa.quantity - 1))}
                className="btn-secondary p-2"
              >
                <Minus size={16} />
              </button>
              <span className="font-bold w-8 text-center">{sobremesa.quantity}</span>
              <button 
                onClick={() => updateQuantity(sobremesa.id, sobremesa.quantity + 1)}
                className="btn-secondary p-2"
              >
                <Plus size={16} />
              </button>
            ...
            <span className="text-accent font-bold">
              R$ {(sobremesa.price * sobremesa.quantity).toFixed(2).replace('.', ',')}
            </span>
          </div>
        </div>
      );
      ...
      <Layout>
        <div className="container mx-auto px-4 py-8">
          <div className="text-center py-12">
            ...
            <p className="text-muted-foreground mb-6">Adicione itens para fazer seu pedido</p>
    ...
    <Layout>
      <div className="container mx-auto px-4 py-4">
        ...
        <h1 className="text-2xl font-bold mb-6">
          Carrinho <span className="text-muted-foreground">({getCartCount()})</span>
        </h1>
